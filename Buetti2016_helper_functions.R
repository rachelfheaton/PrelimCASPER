...
get_accuracy_df <- function(df, remove_timeouts=TRUE){
  ...
  out_df <- df %>% 
    ...
    filter(RT > 300) %>%
    group_by(Subject) %>% 
    summarise(accuracy = 100 - round(mean(Error) * 100, 2))
}
...
get_clean_df <- function(df, accuracy_df, min_accuracy=90){
  ...
  bad_subs <- (accuracy_df %>%
                 filter(accuracy < min_accuracy))$Subject
  print(bad_subs)
  
  good_subs_df <- df %>% 
    filter(!is.element(Subject, bad_subs))
  
  correct_trials_df <- good_subs_df %>% 
    filter(Error == 0, RT > 300)
  ...
  clean_df <- correct_trials_df %>%
    ...
    mutate(meanRT = mean(RT),
           sdRT = sd(RT)) %>% 
    #mutate(outlier = if_else(RT < meanRT - (2.5*sdRT) | RT > meanRT + (2.5*sdRT), 1, 0)) %>% 
    #filter(outlier == 0) 
  ...
  print(paste("Total participants: ",
              length(unique(clean_df$Subject))))
  return(clean_df)    
...
get_plot_df <- function(df, iv_str){
  ...
  out_df <- df %>%
    group_by(numDistractors, !!iv_var) %>%
    summarise(RT = mean(meanRT),
              SEM = sd(meanRT)/sqrt(length(unique(df$Subject))))

  target_only <- out_df %>%
    filter(numDistractors == 0)
  ...
  out_df <- rbind(out_df, target_only) %>%
    distinct(numDistractors, !!iv_var, .keep_all = TRUE) %>%
    filter(distractorColors > 0) %>%
    mutate(plot_color = ifelse(numDistractors==0, 99, !!iv_var))
...
get_dotplot_df <- function(df, iv_str){
    ...
    out_df <- df %>% group_by(numDistractors, !!iv_var)
...
plot_log_curves <- function(df, iv_str, line_names){
  ...
  ggplot(df, aes(x=numDistractors, y=RT, color=!!iv_var)) +
   ...
   # geom_jitter(data=dotplot_df, aes(color=plot_color), size=4) +
    scale_color_manual(values=c('#2abd73','#f6be00', '#f06a11', '#d4240d'),
                        labels = line_names)+
    ...
    scale_y_continuous( limits=c(400,800)) + scale_x_continuous(breaks=c(1,5,10,20,32)) +
...
plot_scatter_curves <- function(df, iv_str, line_names){
  ...
  ggplot(df, aes(x=numDistractors, y=meanRT, color=!!iv_var)) +
    geom_jitter(data=df, aes(color=plot_color), size=1) +
    #geom_errorbar(aes(ymin=RT-SEM, ymax=RT+SEM, color=plot_color), width=1.2, size=1.2) + 
