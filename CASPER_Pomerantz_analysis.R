setwd("~/AttentionModel Add Relations/DissertationCASPER/data/Pom16bits")
source('~/AttentionModel Add Relations/DissertationCASPER/CASPER_Pomerantz_helper_functions.R')
library(tidyverse)
...
plot_log_curves(plot_df, 'plot_color',
                c("None", "Poor", "Good", "Target Only"))#, "Feature Only", "Target Only"))
...
print(log_df$log_fit)



lapply(log_df$log_fit, summary.lm)
log_summary_df <- lapply(log_df$log_fit, summary.lm)
print(log_summary_df)
log_out <- lapply(log_summary_df , function(x) x$r.squared )


human_data <- read.csv("~/AttentionModel Add Relations/DissertationCASPER/PomerantzEstimates_formattedforCASPER.csv", header=TRUE)
...
# 
# max_human_setsize = max(human_df$total_setsize)
# min_human_setsize = min(human_df$total_setsize)
# max_model_setsize = max(plot_df$total_setsize)
# min_model_setsize = min(plot_df$total_setsize)
# 
# human_RTs <- human_df %>%
#   group_by(dcolor) %>%
...
max_human_setsize = max(human_df$total_setsize)
max_model_setsize = max(nontargetonly_df$total_setsize)
min_human_setsize = min(human_df$total_setsize)
min_model_setsize = min(nontargetonly_df$total_setsize)
...
cond1_human_df <- filter(human_df, dcolor==1)
cond2_human_df <- filter(human_df, dcolor==2)
cond3_human_df <- filter(human_df, dcolor==3)

cond1_model_df <- filter(nontargetonly_df, dcolor==1)
cond2_model_df <- filter(nontargetonly_df, dcolor==2)
cond3_model_df <- filter(nontargetonly_df, dcolor==3)
...
all_rsq <- rsq(human_df$RT, nontargetonly_df$RT)

plot(human_df$RT, nontargetonly_df$RT)
